          ...
          <div class="space-y-2">
            ...
            <label class="flex items-center">
              <input
                ...
                class="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300 rounded"
              />
              <span class="ml-2 text-gray-700">多选题</span>
            </label>
            <label class="flex items-center">
              <input
                v-model="settings.types"
...
onMounted(async () => {
  ...
  // 检查路由参数，如果指定了类型，则自动开始练习
  if (route.query.type) {
    settings.value.types = [route.query.type]
    await startPractice()
  }
})
...
const startPractice = async () => {
  try {
    // 如果只选择了一种类型，传递type参数
    const params = { count: settings.value.count }
    if (settings.value.types.length === 1) {
      params.type = settings.value.types[0]
    }
    const data = await getRandomQuestions(settings.value.count, params.type)
